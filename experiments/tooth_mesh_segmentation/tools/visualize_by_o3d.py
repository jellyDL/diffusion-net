...
def main():
    ...
    # 使用matplotlib 建立二维图片坐标系，依次读入4张png图片，每行2张图，每个图像下方标注time_steps值，最后保存为一张整图
    images = []
    for i in range(0, 4):
        ...
        if os.path.exists(img_path):
            ...
            print(f"警告: 图像文件 '{img_path}' 不存在，跳过该图像")
    if not images:
        print("错误: 没有找到任何有效的图像文件")
        return
    # 创建一个新的图形
    fig, axs = plt.subplots(2, 2, figsize=(12, 10))
    axs = axs.flatten()  # 将2D数组展平为1D数组，方便迭代       
    for i, ax in enumerate(axs):
        if i < len(images):
            ...
            ax.axis('off')  # 如果没有图像，关闭坐标轴
    plt.tight_layout()
    output_combined_file = "combined_rendered_views.png"
    plt.savefig(output_combined_file, dpi=800)
    print(f"已保存合并后的图片: {output_combined_file}")
